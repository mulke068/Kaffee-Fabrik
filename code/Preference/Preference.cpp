...
bool Preference::begin(const char * name, bool readOnly, const char* partition_label){
    if(_started){
        return false;
    }
    _readOnly = readOnly;
...
/*
 * Clear all keys in opened Preference
 * */

bool Preference::clear(){
...
bool Preference::remove(const char * key){
...
size_t Preference::putChar(const char* key, int8_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putUChar(const char* key, uint8_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putShort(const char* key, int16_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putUShort(const char* key, uint16_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putInt(const char* key, int32_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putUInt(const char* key, uint32_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putLong(const char* key, int32_t value){
    return putInt(key, value);
}

size_t Preference::putULong(const char* key, uint32_t value){
    return putUInt(key, value);
}

size_t Preference::putLong64(const char* key, int64_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putULong64(const char* key, uint64_t value){
    if(!_started || !key || _readOnly){
        return 0;
    }
...
size_t Preference::putFloat(const char* key, const float_t value){
    return putBytes(key, (void*)&value, sizeof(float_t));
}

size_t Preference::putDouble(const char* key, const double_t value){
    return putBytes(key, (void*)&value, sizeof(double_t));
}

size_t Preference::putBool(const char* key, const bool value){
    return putUChar(key, (uint8_t) (value ? 1 : 0));
}

size_t Preference::putString(const char* key, const char* value){
...
size_t Preference::putString(const char* key, const String value){
    return putString(key, value.c_str());
}

size_t Preference::putBytes(const char* key, const void* value, size_t len){
...
PreferenceType Preference::getType(const char* key) {
...
bool Preference::isKey(const char* key) {
...
int8_t Preference::getChar(const char* key, const int8_t defaultValue){
...
uint8_t Preference::getUChar(const char* key, const uint8_t defaultValue){
...
int16_t Preference::getShort(const char* key, const int16_t defaultValue){
...
uint16_t Preference::getUShort(const char* key, const uint16_t defaultValue){
...
int32_t Preference::getInt(const char* key, const int32_t defaultValue){
...
uint32_t Preference::getUInt(const char* key, const uint32_t defaultValue){
...
int32_t Preference::getLong(const char* key, const int32_t defaultValue){
    return getInt(key, defaultValue);
}

uint32_t Preference::getULong(const char* key, const uint32_t defaultValue){
    return getUInt(key, defaultValue);
}

int64_t Preference::getLong64(const char* key, const int64_t defaultValue){
...
uint64_t Preference::getULong64(const char* key, const uint64_t defaultValue){
...
float_t Preference::getFloat(const char* key, const float_t defaultValue) {
    float_t value = defaultValue;
    getBytes(key, (void*) &value, sizeof(float_t));
    return value;
}

double_t Preference::getDouble(const char* key, const double_t defaultValue) {
    double_t value = defaultValue;
    getBytes(key, (void*) &value, sizeof(double_t));
    return value;
}

bool Preference::getBool(const char* key, const bool defaultValue) {
    return getUChar(key, defaultValue ? 1 : 0) == 1;
}

size_t Preference::getString(const char* key, char* value, const size_t maxLen){
    size_t len = 0;
...
String Preference::getString(const char* key, const String defaultValue){
    char * value = NULL;
    size_t len = 0;
    if(!_started || !key){
...
size_t Preference::getBytesLength(const char* key){
    size_t len = 0;
    if(!_started || !key){
        return 0;
    }
...
size_t Preference::getBytes(const char* key, void * buf, size_t maxLen){
